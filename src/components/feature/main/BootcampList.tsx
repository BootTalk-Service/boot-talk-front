...
const BootcampList = ({ filters }: BootcampListProps) => {
  ...
  const filtered = bootcamps?.filter((camp: Bootcamp) => {
    if (filters["지역"] && !camp.region.startsWith(filters["지역"])) return false;
    if (filters["직무"] && !(camp.Category || []).includes(filters["직무"])) return false;
    if (filters["기간"]) {
      const start = new Date(camp.bt_startDate);
      const end = new Date(camp.bt_endDate);
    ...
    if (filters["평점"]) {
      ...
      if (camp.bt_rating < rating) return false;
    }
  ...
  return (
    <section className="px-28 py-6">
      ...
      <div className="grid grid-cols-6 gap-4 px-4 py-2 font-semibold text-sm text-gray-600 border-b border-t border-slate-300 bg-slate-50">
        <span>교육과정명</span>
        <span>학습기간</span>
        <span className="flex justify-start pl-6">프로그램 과정</span>
        <span>지역</span>
